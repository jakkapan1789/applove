import { AskingOption, ConfirmOption, GeneralOption, LoveOption } from './interfaces/options'
...
export const applove = {
  ...
  asking: ({ title = "Please confirm", confirmButtonLabel = "Confirm", cancelButtonLabel = "Cancel", type = "question", iconColor = "currentColor", fontFamily = "'Kanit', sans-serif", errorLabel = "Input cannot be empty." }: AskingOption): Promise<Response> => {
    return new Promise<Response>((resolve, reject) => {
      document.body.style.fontFamily = fontFamily;
      const overlay = document.createElement("div");
      overlay.style.cssText = popupOverlay;
      ...
      trueButton.addEventListener("click", () => {
        if (inputBox.value.trim() === "") {
          ...
          const response: Response = { result: true, detail: inputBox.value };
          resolve(response);
          close();
        }
      });
